...
public class MoneyCommands implements CommandExecutor, TabCompleter {
    ...
    private void handlePay(CommandSender sender, String[] args) {
        ...
        if (!CoreAPI.hasMoney(player, amount)) {
            sender.sendMessage(localeManager.getComponent("money.pay.insufficient_funds", moneyFormat.format(amount)));
            return;
        }

        CoreAPI.removeMoney(player, amount);
        CoreAPI.addMoney(target, amount);

        double senderBalance = CoreAPI.getMoney(player);
        double targetBalance = CoreAPI.getMoney(target);

        sender.sendMessage(localeManager.getComponent("money.pay.sent", 
            moneyFormat.format(amount), target.getName(), moneyFormat.format(senderBalance)));
        target.sendMessage(localeManager.getComponent("money.pay.received", 
            moneyFormat.format(amount), player.getName(), moneyFormat.format(targetBalance)));
    }
        ...
        switch (command.getName().toLowerCase()) {
            ...
            case "pay" -> {
                if (args.length == 1) {
                    ...
                    List<String> amounts = List.of("100", "1000", "10000");
                    amounts.stream()
