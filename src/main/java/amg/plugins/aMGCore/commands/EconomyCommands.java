...
public class EconomyCommands implements CommandExecutor, TabCompleter {
    ...
    private boolean handlePay(CommandSender sender, String[] args) {
        if (!(sender instanceof Player player)) {
            sender.sendMessage(localeManager.getComponent("money.pay.console_denied"));
            return true;
        }

        if (args.length != 2) {
            sender.sendMessage(localeManager.getComponent("money.pay.usage"));
            return true;
        }
        ...
        if (targetPlayer == player) {
            sender.sendMessage(localeManager.getComponent("money.pay.self"));
            return true;
        }
        ...
        if (senderData == null) {
            sender.sendMessage(localeManager.getComponent("money.error.data_load"));
            return true;
        }

        if (!senderData.hasMoney(amount)) {
            sender.sendMessage(localeManager.getComponent("money.pay.insufficient_funds", formatMoney(amount)));
            return true;
        }
        ...
        // Notify both players
        player.sendMessage(localeManager.getComponent("money.pay.sent", 
            formatMoney(amount), targetPlayer.getName(), formatMoney(senderData.getMoney())));
        targetPlayer.sendMessage(localeManager.getComponent("money.pay.received", 
            formatMoney(amount), player.getName(), formatMoney(targetData.getMoney())));
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (command.getName().equalsIgnoreCase("money") || 
            ...
            if (args.length == 1) {
                // Tab complete player names (excluding sender)
                return getOnlinePlayerNamesExcludingSender(args[0], sender);
            } else if (args.length == 2) {
                // Suggest common amounts
                String partial = args[1].toLowerCase();
                List<String> amounts = Arrays.asList("100", "1000", "10000");
    ...
    private List<String> getOnlinePlayerNamesExcludingSender(String prefix, CommandSender sender) {
        String lowerPrefix = prefix.toLowerCase();
        String senderName = sender instanceof Player ? sender.getName() : "";
        return Bukkit.getOnlinePlayers().stream()
            .map(Player::getName)
            .filter(name -> !name.equals(senderName)) // Exclude sender
            .filter(name -> name.toLowerCase().startsWith(lowerPrefix))
            .collect(Collectors.toList());
    }
